...
function stringsToNumbs(numbers) {return numbers.map(str => parseInt(str))}
...
function sumEvens() {
    let strNums2 = stringsToNumbs(strNums)
    let total = 0
    for(let i = strNums2.length; i >= 0; i--){
        if (strNums2[i]%2 == 0) {
            total += strNums2[i]
        }
    }
...
function weatherStates(weather) {
    stateNames = weather.map(m=>m.weather_state_name)
...
function weatherTemps(weather) {
    temps = weather.map(m=>m.min_temp)
    
    for(let i = temps.length; i > -1; i--) {
        if (temps[i] >= 15.915) {
            return weather[i]
        }
    }
...
console.log(weatherTemps(weather))